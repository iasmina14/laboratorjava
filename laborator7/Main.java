package laborator7;

import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;
import java.util.Locale;
import java.util.concurrent.atomic.AtomicReference;
import java.util.stream.Collectors;

public class Main {
    public static void main(String[] args) {
        problema_var1();
        List<String> inputs = Arrays.asList("lalea", "bujor", "iasomie");
        System.out.println(upperofList(inputs));
    }

    //calculati media aritmetica a numerelor intregi intr-o lista
    public static void problema_var1() {
        List<Integer> numereIntregi = Arrays.asList(3, 6, 8, 43, 65, 23, 76);
        AtomicReference<Integer> sum = new AtomicReference<>(0);
        numereIntregi.forEach(n -> {
            sum.set(sum.get() + n);
        });

        System.out.println(sum.get() / numereIntregi.size());
    }

    public static void problema1_var2(){
        List<Integer> numereIntregi = Arrays.asList(3, 6, 8, 43, 65, 23, 76);
        double v = numereIntregi.stream()
                .mapToDouble(i -> i.doubleValue())
                .average()
                .orElse(0.0);
        System.out.println(v);
    }

    //convertiti o lista de string in upperCase
    public static List<String>upperofList(List<String> lista){
        lista.forEach(e -> {
            lista.set(lista.indexOf(e), e.toUpperCase() );
        });
        return lista;
    }

    public static void problema2_var2(){
        List<String> inputs = Arrays.asList("lalea", "bujor", "iasomie");
        List<String> newList = inputs.stream()
                .map(l->l.toUpperCase())
                .collect(Collectors.toList());
        System.out.println(newList);
    }
}




